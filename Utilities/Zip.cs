...
public class Zip
{
            ...
            foreach (var execName in executableNames)
            {
                try
                {
                    ...
                    if (result != null)
                    {
                        ...
                        {
                            Logger.Log($"Found {execName} in system PATH: {pathFromWhich}");
    ...
    private static async Task<string> RunSevenZipCommand(string arguments)
    {
        ...
        process.OutputDataReceived += (_, e) => { if (e.Data != null)
            {
                output.AppendLine(e.Data);
            }
        };
        process.ErrorDataReceived += (_, e) => { if (e.Data != null)
            {
    ...
    public static async Task ExtractArchive(string archivePath, string destinationPath, string password = null)
    {
        ...
        Logger.Log($"Extracting {archivePath} to {destinationPath} using 7z executable...");
        var output = await RunSevenZipCommand(arguments);
        Logger.Log($"Extraction complete - 7z output: {output.Split('\n').Length} lines");
    }
    ...
    public static async Task CreateArchive(string archivePath, string sourcePath)
    {
        ...
        Logger.Log($"Creating archive {archivePath} from {sourcePath}...");
        var output = await RunSevenZipCommand(arguments);
        Logger.Log($"Archive created successfully - 7z output: {output.Split('\n').Length} lines");
    }
}