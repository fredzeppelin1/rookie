    ...
    public partial class QuestOptionsWindow : Window
    {
        ...
        private async void TransferScreenshotsButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var sourcePath = "/sdcard/Oculus/Screenshots/";
                var pullResult = await Task.Run(() => Adb.RunAdbCommandToString($"pull \"{sourcePath}\" \"{screenshotsPath}\""));
        ...
        private async void TransferRecordingsButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var sourcePath = "/sdcard/Oculus/VideoShots/";
                var pullResult = await Task.Run(() => Adb.RunAdbCommandToString($"pull \"{sourcePath}\" \"{recordingsPath}\""));
        ...
        private void RefreshRateComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (comboBox == null || comboBox.SelectedIndex <= 0 || string.IsNullOrEmpty(Adb.DeviceId))
                return;
        ...
        private void GpuLevelComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            var comboBox = sender as ComboBox;
            if (comboBox == null || comboBox.SelectedIndex <= 0 || string.IsNullOrEmpty(Adb.DeviceId))
                return;
        ...
        private void CpuLevelComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            var comboBox = sender as ComboBox;
            if (comboBox == null || comboBox.SelectedIndex <= 0 || string.IsNullOrEmpty(Adb.DeviceId))
                return;
